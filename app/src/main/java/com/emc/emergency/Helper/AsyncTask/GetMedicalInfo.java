...
public class GetMedicalInfo extends AsyncTask<Void, Void, ArrayList<Medical_Information>> {
    ...
    public GetMedicalInfo(Activity activity, ArrayList<Medical_Information> arrMI, int medical_type, RecyclerView.Adapter adapter, Long id_victim) {
    ...
    protected ArrayList<Medical_Information> doInBackground(Void... params) {
        ...
        if (id_victim == -99L) {
            ...
            id = sharedPreferences.getLong("id_PI", -1);
        }
        ...
        try {
            URL url = null;
            if (id_victim == -99L)
                url = new URL(SystemUtils.getServerBaseUrl() + "personal_Infomations/" + id + "/medical_Info");
            else
                url = new URL(SystemUtils.getServerBaseUrl() + "personal_Infomations/" + id_victim + "/medical_Info");
